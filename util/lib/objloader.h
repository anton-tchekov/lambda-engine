...
int tinyobj_parse_obj(char *path, char *name, tinyobj_attrib_t *attrib, tinyobj_shape_t **shapes,
											int *num_shapes, tinyobj_material_t **materials_out,
											int *num_materials_out, const char *buf, int len,
											unsigned int flags);

int tinyobj_parse_mtl_file(tinyobj_material_t **materials_out,
																	int *num_materials_out,
																	const char *filename);

void tinyobj_attrib_init(tinyobj_attrib_t *attrib);
void tinyobj_attrib_free(tinyobj_attrib_t *attrib);
void tinyobj_shapes_free(tinyobj_shape_t *shapes, int num_shapes);
void tinyobj_materials_free(tinyobj_material_t *materials, int num_materials);
...
#define TINYOBJ_MAX_FACES_PER_F_LINE 1024
...
static int length_until_line_feed(const char *token, int n) {
	int len = 0;
...
static int my_strnlen(const char *s, int n) {
...
char *dynamic_fgets(char **buf, int *size, FILE *file) {
	...
	do {
		...
		*buf = (char*)realloc(*buf, *size);
		...
		ret = fgets(offset, (old_size + 1), file);
...
static void initMaterial(tinyobj_material_t *material) {
	...
	material->emissive_texname = NULL;
...
static void create_hash_table(int start_capacity, hash_table_t* hash_table)
{
	if (start_capacity < 1)
...
{
	/* Insert value */
	int start_index = hash % hash_table->capacity;
...
static void hash_table_maybe_grow(int new_n, hash_table_t* hash_table)
{
	int new_capacity;
...
static void hash_table_set(const char* name, int val, hash_table_t* hash_table)
{
	/* Hash name */
...
static int tinyobj_parse_and_index_mtl_file(tinyobj_material_t **materials_out,
	...
	while (NULL != dynamic_fgets(&linebuf, &buffer_size, fp)) {
		...
		/* nonstandard? emissive texture */
		if ((0 == strncmp(token, "map_Ke", 6)) && IS_SPACE(token[6])) {
			token += 7;
			material.emissive_texname = my_strdup(token,  (line_end - token));
			continue;
		}
...
static int parseLine(Command *command, const char *p, int p_len,
										 int triangulate) {
	char linebuf[4096];
	const char *token;
	assert(p_len < 4095);
	...
	if ((0 == strncmp(token, "usemtl", 6)) && IS_SPACE((token[6]))) {
		...
		command->material_name_len = length_until_newline(
																																		token, (p_len - (token - linebuf)) + 1);
	...
	if ((0 == strncmp(token, "mtllib", 6)) && IS_SPACE((token[6]))) {
		...
		command->mtllib_name_len = length_until_newline(
	...
	if (token[0] == 'g' && IS_SPACE((token[1]))) {
		...
		command->group_name_len = length_until_newline(
																																 token, p_len - (token - linebuf)) +
			1;
	...
	if (token[0] == 'o' && IS_SPACE((token[1]))) {
		...
		command->object_name_len = length_until_newline(
																																	token, p_len - (token - linebuf)) +
			1;
...
static int is_line_ending(const char *p, int i, int end_i) {
	if (p[i] == '\0') return 1;
...
int tinyobj_parse_obj(char *path, char *name, tinyobj_attrib_t *attrib, tinyobj_shape_t **shapes,
	 ...
	 /* 1. Find '\n' and create line data. */
	{
		...
		line_infos = malloc(sizeof(LineInfo) * num_lines);
	...
	commands = malloc(sizeof(Command) * num_lines);
	...
	/* Load material(if exits) */
	{
		...
		{
			strncpy(name, commands[mtllib_line_index].mtllib_name, commands[mtllib_line_index].mtllib_name_len);
			int ret = tinyobj_parse_and_index_mtl_file(&materials, &num_materials, path, &material_table);
...

	{
		...
		attrib->vertices = malloc(sizeof(float) * num_v * 3);
		attrib->num_vertices = num_v;
		attrib->normals = malloc(sizeof(float) * num_vn * 3);
		attrib->num_normals = num_vn;
		attrib->texcoords = malloc(sizeof(float) * num_vt * 2);
		attrib->num_texcoords = num_vt;
		attrib->faces = malloc(sizeof(tinyobj_vertex_index_t) * num_f);
		attrib->num_faces = num_f;
		attrib->face_num_verts = malloc(sizeof(int) * num_faces);
		attrib->material_ids = malloc(sizeof(int) * num_faces);
	...
	/* 5. Construct shape information. */
	{
		...
		*shapes = malloc(sizeof(tinyobj_shape_t) * (n + 1));
		...
		if ((face_count - prev_face_offset) > 0) {
			int length = face_count - prev_shape_face_offset;
			if (length > 0) {
				(*shapes)[shape_idx].name =
					my_strndup(prev_shape_name, prev_shape_name_len);
				(*shapes)[shape_idx].face_offset = prev_face_offset;
...
void tinyobj_materials_free(tinyobj_material_t *materials,
	...
	for (i = 0; i < num_materials; i++) {
		...
		if (materials[i].emissive_texname) free(materials[i].emissive_texname);
